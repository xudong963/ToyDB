...
public class JoinOptimizer {
     ...
     * @return The cardinality of the join(The number of tuples output by join j)
     */
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for(int i=1; i<=joinNum; i++)
        {
            ...
            for(Set<LogicalJoinNode> set : subset)
            {
                ...
                {
                    CostCard costCard = this.computeCostAndCardOfSubplan(stats, filterSelectivities, logicalJoinNode, set, bestCost, planCache);
                    if (costCard==null)
                        continue;
